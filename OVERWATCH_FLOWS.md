...
| Family | Columns |
|---|---|
| Identity | `id, p_number, s_number, registry_id` — **`job_number` and `parent_job_id` DO NOT EXIST on the live table.** They appear throughout the code; both were documented here from source rather than from the database. Verified against `information_schema`: 83 columns, neither present. |
...
(`supabase.js:427`, `api/sse.js:407`) — **into a column that does not exist**, so every path
that sets it throws rather than racing.
...
`jobsApi.createLinkedJob` (`supabase.js:427`) attempt a **child job** with `parent_job_id` —
a column that does not exist, so neither has ever succeeded —
...
10. **`job_number` and `parent_job_id` do not exist on `jobs`** — corrected 2026-08-20 against
    the live schema. This is not a race, as originally filed; it is a hard failure. Every path
    that writes either column throws on an unknown column:
    `jobsApi.createLinkedJob` · `getLinkedJobs` · `getJobWithFamily` · `getTotalJobValue` ·
    MCP `create_job` · MCP `create_return_visit` · MCP `get_job_family`.
    **Three of the seventeen MCP tools are dead on arrival, and the entire job-linking API with
    them** — which is why return visits land as loose cards. The parent/child relationship is
    real in the work; the schema has nowhere to record it, so related cards are indistinguishable
    from duplicates. `p_number` works (69 rows, DB trigger); `s_number` has never produced a value.
