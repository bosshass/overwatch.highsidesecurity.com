// ============================================
// BoardView — NakedPM rebuild
// ============================================
// Supabase jobs table is the ONLY source of truth.
// No Google Calendar reads. Calendar writes still happen
// on scheduling (tech cal event creation) but board state
// is driven entirely by jobs.status.
//
// Columns:
//   Triage      → new | needs_details | needs_parts | pending_materials | needs_estimate
//   Ready       → ready_to_schedule
//   Returns     → return_pending
//   Scheduled   → scheduled
//   Estimates   → estimate_sent
//   To Bill     → complete | to_bill
//   Won/Lost    → won | lost
// ============================================

import { useState, useEffect, useCallback, useRef } from 'react';
import { useNavigate } from 'react-router-dom';
import { supabase, JOB_STATUS, STATUS_INFO, techsApi } from '../services/supabase.js';
import NewJobModal from '../components/NewJobModal.jsx';

const GCAL = 'https://www.googleapis.com/calendar/v3';

// Status → next legal moves (NakedPM verb set)
const STATUS_VERBS = {
...
const COLUMNS = [
  {
    ...
    label: '🔥 Triage',
    color: '#ef4444',
    statuses: ['new', 'needs_details', 'needs_parts', 'pending_materials', 'needs_estimate'],
  },
  {
    key: 'ready',
    label: '✅ Ready',
...
// ── Note input component (inline, required before status move) ──────────────
function NoteInput({ onSubmit, onCancel, loading }) {
  ...
  return (
    <div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 6 }} onClick={e => e.stopPropagation()}>
      <textarea
        ...
        rows={2}
        style={{
          width: '100%', padding: 8, borderRadius: 6, border: '1px solid #334155',
          background: '#0f172a', color: '#fff', fontSize: 12, fontFamily: 'inherit',
      ...
      <div style={{ display: 'flex', gap: 6 }}>
        <button
          ...
          style={{
            ...
            background: note.trim() ? '#22c55e' : '#334155',
            color: '#fff', fontWeight: 600, fontSize: 12, cursor: note.trim() ? 'pointer' : 'not-allowed',
          }}
        >
          {loading ? '…' : 'confirm'}
        </button>
        <button
          onClick={onCancel}
          style={{
            flex: 1, padding: '7px 0', borderRadius: 6, border: '1px solid #334155',
...
// ── Scheduler modal (writes GCal event + updates job.scheduled_date + tech) ──
function SchedulerModal({ job, techs, accessToken, onScheduled, onClose }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      // 2. Create GCal event on tech's calendar (best-effort, non-blocking)
      if (accessToken && selectedTech?.calendar_id) {
        const body = {
          ...
          description: [
            ...
            job.cms_account_id ? `CMS: ${job.cms_account_id}` : '',
            notes ? `\nNotes: ${notes}` : '',
          ].filter(Boolean).join('\n'),
        ...
        await fetch(
          `${GCAL}/calendars/${encodeURIComponent(selectedTech.calendar_id)}/events`,
          {
            method: 'POST',
  ...
  return (
    <div
      style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', zIndex: 1100, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}
      onClick={onClose}
    >
      <div
        style={{ background: '#1e293b', borderRadius: '16px 16px 0 0', width: '100%', maxWidth: 520, padding: '20px 20px 32px', maxHeight: '90vh', overflowY: 'auto' }}
      ...
      >
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 16 }}>
          {techs.map(t => (
            <button
              ...
              style={{
                padding: '8px 14px', borderRadius: 8, border: `2px solid ${techId === t.id ? t.color : '#334155'}`,
                background: techId === t.id ? `${t.color}22` : '#0f172a',
                color: techId === t.id ? t.color : '#64748b', fontWeight: 600, cursor: 'pointer', fontSize: 13,
              }}
            >
              {t.name}
            </button>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button
            onClick={submit} disabled={saving || !techId || !date}
...
// ── Detail drawer ─────────────────────────────────────────────────────────────
function DetailDrawer({ job, techs, accessToken, onStatusMove, onSchedule, onClose, moving }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', zIndex: 1000, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }} onClick={onClose}>
      <div style={{ background: '#1e293b', borderRadius: '16px 16px 0 0', width: '100%', maxWidth: 520, padding: '20px 20px 40px', maxHeight: '85vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
        <div style={{ width: 36, height: 4, background: '#334155', borderRadius: 2, margin: '0 auto 16px' }} />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px 16px', fontSize: 12, marginBottom: 14 }}>
          ...
          {job.customer_id === null && (
            <div style={{ gridColumn: '1/-1', background: '#451a1a', borderRadius: 6, padding: '6px 10px', fontSize: 11, color: '#fca5a5' }}>
              ⚠ no customer UUID linked
            </div>
          )}
        </div>
        ...
        {/* Schedule button — shown when ready_to_schedule or return_pending */}
        {(job.status === 'ready_to_schedule' || job.status === 'return_pending') && (
          <button
            onClick={() => { setPendingVerb(null); onSchedule(job); }}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {verbs.filter(v => v !== 'scheduled').map((verb, i) => {
            const vsi = STATUS_INFO[verb] || {};
            const isPrimary = i === 0;
            return pendingVerb === verb ? (
              <NoteInput
                key={verb}
                onSubmit={note => { onStatusMove(job.id, verb, note); setPendingVerb(null); }}
              ...
              <button
                key={verb}
                onClick={() => setPendingVerb(verb)}
                disabled={moving}
                style={{
                  padding: 12, borderRadius: 8, border: isPrimary ? 'none' : `1px solid ${vsi.color || '#334155'}`,
                  background: isPrimary ? (vsi.color || '#334155') : 'transparent',
                  color: isPrimary ? '#fff' : (vsi.color || '#94a3b8'),
  ...
  return (
    <div
      ...
      style={{
        ...
        padding: 12,
        marginBottom: 8,
        borderLeft: `3px solid ${isUrgent ? '#ef4444' : (si.color || '#334155')}`,
        cursor: 'pointer',
        opacity: job.status === 'dead' || job.status === 'lost' ? 0.55 : 1,
      }}
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8, marginBottom: 4 }}>
        <div style={{ fontSize: 14, fontWeight: 500, color: '#fff', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          {job.customer_name || '—'}
        </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 6 }}>
        <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
          ...
          <span style={{ fontSize: 10, color: '#334155' }}>· {dago(job.created_at)}</span>
        </div>
        {quickVerbs.length > 0 && (
          <button
            onClick={e => { e.stopPropagation(); onQuickMove(job, quickVerbs[0]); }}
            disabled={moving}
            style={{
              padding: '3px 8px', borderRadius: 5, border: `1px solid ${STATUS_INFO[quickVerbs[0]]?.color || '#334155'}`,
              background: 'transparent', color: STATUS_INFO[quickVerbs[0]]?.color || '#94a3b8',
    ...
    <div style={{ flex: 1, minWidth: 260, maxWidth: 340, display: 'flex', flexDirection: 'column' }}>
      ...
      <div
        ...
        style={{
          ...
          borderBottom: `3px solid ${col.color}`, borderRadius: '8px 8px 0 0',
          cursor: 'pointer',
        }}
      >
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 10, background: '#0f172a', borderRadius: '0 0 8px 8px' }}>
        {jobs.length === 0
          ...
          : jobs.map(j => (
            <JobCard key={j.id} job={j} onSelect={onSelect} onQuickMove={onQuickMove} moving={moving} />
          ))
        }
      </div>
    </div>
...
export default function BoardView({ accessToken, onBack, userEmail, userName }) {
  ...
  // ── Load all open jobs from Supabase ────────────────────────────────────────
  ...
  const loadTechs = useCallback(async () => {
    try {
      const t = await techsApi.getAll();
      setTechs(t);
    } catch (e) {
      console.warn('techs load failed:', e);
    }
  }, []);
  ...
  // ── Status move ─────────────────────────────────────────────────────────────
  const moveStatus = useCallback(async (jobId, newStatus, note) => {
    setMoving(true);
    try {
      ...
      setJobs(prev => prev.map(j => j.id === jobId ? { ...j, status: newStatus, action_note: note } : j));
  ...
  // Quick move (no note required for non-destructive single-hop moves)
  const quickMove = useCallback(async (job, verb) => {
    await moveStatus(job.id, verb, `quick move → ${verb} via board`);
  }, [moveStatus]);

  // ── Filter + bucket jobs into columns ───────────────────────────────────────
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0f172a', color: '#fff', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', borderBottom: '1px solid #1e293b' }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button
            ...
            style={{ background: '#22c55e', border: 'none', color: '#fff', padding: '8px 16px', borderRadius: 8, cursor: 'pointer', fontWeight: 600, fontSize: 13 }}
          >
            + New Job
          </button>
          <button onClick={loadJobs} disabled={loading} style={{ background: '#1e293b', border: 'none', color: '#94a3b8', padding: '8px 14px', borderRadius: 8, cursor: 'pointer', fontSize: 13 }}>
            {loading ? '…' : '↻'}
          </button>
        </div>
      </div>
      ...
      {/* Stats bar */}
      <div style={{ display: 'flex', gap: 10, padding: '10px 16px', borderBottom: '1px solid #1e293b', flexWrap: 'wrap' }}>
        {[
          { label: 'open', val: stats.total_open, color: '#64748b' },
          { label: 'needs action', val: stats.needs_action, color: '#ef4444' },
          { label: 'to bill', val: stats.to_bill, color: '#8b5cf6' },
          { label: 'returns', val: stats.returns_pending, color: '#06b6d4' },
        ].map(s => (
        ...
        <input
          ...
          placeholder="search customer, issue, CMS…"
          style={{
        ...
        {COLUMNS.map(col => (
          <button
            ...
            style={{
              ...
              color: activeCol === col.key ? col.color : '#475569',
              cursor: 'pointer', fontSize: 12, fontWeight: activeCol === col.key ? 700 : 400,
            }}
          >
      ...
      {/* New job modal — reuse existing component */}
      {showNewJob && (
        <NewJobModal
          accessToken={accessToken}
          userEmail={userEmail}
      ...
      {toast && (
        <div style={{
          ...
          padding: '10px 20px', borderRadius: 10, fontSize: 13, fontWeight: 500, zIndex: 9999,
