// ============================================
// BoardView — NakedPM v2
// ============================================
// Supabase jobs table = only SOT.
// Fixes in v2:
//   1. UUID linker inline in detail drawer
//   2. Customer search → link or create
//   3. Status verbs work WITHOUT forcing scheduler
//   4. Scheduler is optional (available on ready/return cards)
// ============================================
...
const dago = iso => { if(!iso) return ''; const d=Math.floor((Date.now()-new Date(iso).getTime())/86400000); if(d===0)return'today'; if(d===1)return'1d ago'; if(d<0)return`in ${-d}d`; return`${d}d ago`; };
  ...
  return (
    <div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 6 }} onClick={e => e.stopPropagation()}>
      <textarea ref={ref} value={note} onChange={e => setNote(e.target.value)}
        placeholder="note required…" rows={2}
        style={{ width:'100%', padding:8, borderRadius:6, border:'1px solid #334155', background:'#0f172a', color:'#fff', fontSize:12, fontFamily:'inherit', resize:'none', boxSizing:'border-box' }}
      />
      <div style={{ display:'flex', gap:6 }}>
        <button onClick={() => note.trim() && onSubmit(note.trim())} disabled={!note.trim()||loading}
          style={{ flex:2, padding:'7px 0', borderRadius:6, border:'none', background:note.trim()?'#22c55e':'#334155', color:'#fff', fontWeight:600, fontSize:12, cursor:note.trim()?'pointer':'not-allowed' }}>
          {loading ? '…' : 'confirm'}
        </button>
        <button onClick={onCancel}
          style={{ flex:1, padding:'7px 0', borderRadius:6, border:'1px solid #334155', background:'transparent', color:'#94a3b8', fontSize:12, cursor:'pointer' }}>
          cancel
        </button>
      </div>
    </div>
  ...
  const submit = async () => {
    ...
    try {
      const { error: dbErr } = await supabase.from('jobs').update({
        ...
        tech_assigned: techId, tech_name: selectedTech?.name || '',
...
// ── Detail drawer ─────────────────────────────────────────────────────────────
function DetailDrawer({ job, techs, accessToken, onStatusMove, onSchedule, onClose, moving, onUUIDLinked }) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.75)', zIndex:1000, display:'flex', alignItems:'flex-end', justifyContent:'center' }} onClick={onClose}>
      <div style={{ background:'#1e293b', borderRadius:'16px 16px 0 0', width:'100%', maxWidth:520, padding:'20px 20px 40px', maxHeight:'90vh', overflowY:'auto' }} onClick={e => e.stopPropagation()}>
        ...
        {/* UUID linker — always show if missing */}
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'6px 16px', fontSize:12, marginBottom:14 }}>
          {[['type',job.job_type],['priority',job.priority],['address',job.customer_address],['phone',job.customer_phone],['CMS',job.cms_account_id],['tech',job.tech_name],['scheduled',job.scheduled_date],['created',dago(job.created_at)]].filter(([,v])=>v).map(([label,val])=>(
            <div key={label}>
        ...
        {/* Optional scheduler button for ready/return */}
        {(job.status==='ready_to_schedule'||job.status==='return_pending') && (
          <button onClick={() => { setPendingVerb(null); onSchedule(job); }}
            ...
            📅 Open Scheduler (optional — pick tech + time)
          </button>
        )}

        {/* Verb buttons — ALL status moves available without scheduler */}
        <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
          {verbs.map((verb, i) => {
            ...
            return pendingVerb === verb ? (
              ...
              <button key={verb} onClick={() => setPendingVerb(verb)} disabled={moving}
                style={{ padding:12, borderRadius:8, border:isPrimary?'none':`1px solid ${vsi.color||'#334155'}`, background:isPrimary?(vsi.color||'#334155'):'transparent', color:isPrimary?'#fff':(vsi.color||'#94a3b8'), fontWeight:600, fontSize:13, cursor:'pointer', textAlign:'left' }}>
...
export default function BoardView({ accessToken, onBack, userEmail, userName }) {
  ...
  const moveStatus = useCallback(async (jobId, newStatus, note) => {
    ...
    try {
      const { error } = await supabase.from('jobs').update({
        status: newStatus, action_note: note||null,
  ...
  const handleUUIDLinked = useCallback((jobId, customerId) => {
    setJobs(prev => prev.map(j => j.id===jobId ? {...j, customer_id:customerId} : j));
    setSelectedJob(prev => prev?.id===jobId ? {...prev, customer_id:customerId} : prev);
    ...
    <div style={{ minHeight:'100vh', background:'#0f172a', color:'#fff', display:'flex', flexDirection:'column' }}>
      ...
      {selectedJob && (
        <DetailDrawer
          ...
          onUUIDLinked={customerId => handleUUIDLinked(selectedJob.id, customerId)}
        />
      )}
