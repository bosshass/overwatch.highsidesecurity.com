    ...
    return (
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '4px' }}>
          ...
          {job.job_number && (
            <span style={{ color: '#e8a33d', fontSize: '11px', fontWeight: '600', flexShrink: 0 }}>{job.job_number}</span>
          )}
        </div>
      ...
      >
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
            ...
            {job.job_number && (
              <span style={{ color: '#e8a33d', fontSize: '10px', fontWeight: '600', flexShrink: 0 }}>{job.job_number}</span>
            )}
          </div>
        ...
        {[
          ...
          <button
            ...
            style={{
              ...
              color: activeTab === t.key ? '#e8a33d' : '#64748b',
              fontSize: '14px', fontWeight: activeTab === t.key ? '700' : '400',
              cursor: 'pointer', position: 'relative',
              borderBottom: activeTab === t.key ? '2px solid #e8a33d' : '2px solid transparent'
            }}
          >
            {t.label}
            {t.badge > 0 && (
              ...
              <div style={{ display: 'flex', gap: '4px', marginBottom: '12px', background: '#16213e', borderRadius: '10px', padding: '3px' }}>
                {[
                  ...
                  <button key={v.key} onClick={() => setBoardView(v.key)} style={{
                    ...
                    color: boardView === v.key ? '#e8a33d' : '#64748b',
                              ...
                              return (
                                <div key={job.id} onClick={() => setSelectedJobId(job.id)} style={{
                                  ...
                                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '3px' }}>
                                    ...
                                    {job.job_number && <span style={{ color: '#e8a33d', fontSize: '10px', fontWeight: 600 }}>{job.job_number}</span>}
                                  </div>
                    ...
                    <div style={{ textAlign: 'center' }}>
                      ...
                      {calendarWeekOffset !== 0 && (
                        <button onClick={() => setCalendarWeekOffset(0)} style={{
                          background: 'none', border: 'none', color: '#e8a33d', fontSize: '11px', cursor: 'pointer', padding: '2px'
                        }}>← Today</button>
                      )}
                    </div>
                    <button onClick={() => setCalendarWeekOffset(o => o + 1)} style={{
                    ...
                    {weekDays.map((day, i) => {
                      ...
                      return (
                        <div key={i} style={{
                          ...
                          background: today ? '#e8a33d15' : '#1e293b', borderRadius: '8px',
                          border: today ? '1px solid #e8a33d' : '1px solid transparent'
                        }}>
                          <div style={{ color: today ? '#e8a33d' : '#94a3b8', fontSize: '10px', fontWeight: 600 }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '50px repeat(7, 1fr)', minWidth: '700px' }}>
                      ...
                      {weekDays.map((day, i) => (
                        <div key={i} style={{
                          ...
                          color: isToday(day) ? '#e8a33d' : '#94a3b8',
                          ...
                          {weekDays.map((day, di) => {
                            ...
                            return (
                              <div key={`${hour}-${di}`} style={{
                                ...
                                background: isToday(day) ? '#e8a33d05' : 'transparent'
                              }}>
                                {jobs.map(job => (
                                  <div key={job.id} onClick={() => setSelectedJobId(job.id)} style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '8px' }}>
                ...
                <div style={{ display: 'flex', gap: '2px', background: '#16213e', borderRadius: '8px', padding: '2px', flexShrink: 0 }}>
                  <button onClick={() => setViewMode('line')} style={{
                    ...
                    padding: '6px 8px', cursor: 'pointer', color: viewMode === 'line' ? '#e8a33d' : '#64748b', fontSize: '14px'
                  ...
                  <button onClick={() => setViewMode('card')} style={{
                    ...
                    padding: '6px 8px', cursor: 'pointer', color: viewMode === 'card' ? '#e8a33d' : '#64748b', fontSize: '14px'
                      ...
                      <div key={job.id} onClick={() => setSelectedJobId(job.id)} style={{
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                          ...
                          <div style={{ textAlign: 'right' }}>
                            <div style={{ color: '#e8a33d', fontSize: '11px', fontWeight: 600 }}>{job.job_number}</div>
                ...
                {customers.map(c => (
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      <div style={{ textAlign: 'right' }}>
                        {jobCounts[c.id] && (
                          <span style={{ color: '#e8a33d', fontSize: '13px', fontWeight: '600' }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                ...
                <button onClick={createCustomer} disabled={!customerForm.name?.trim()} style={{
                  background: customerForm.name?.trim() ? '#e8a33d' : '#334155', color: customerForm.name?.trim() ? '#000' : '#64748b',
              ...
              <div style={{ background: '#1e293b', borderRadius: '12px', padding: '16px', marginBottom: '16px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                  ...
                  <button
                    ...
                    style={{
                      background: isEditingCustomer ? '#ef444420' : '#1e293b', color: isEditingCustomer ? '#ef4444' : '#e8a33d',
                ...
                {isEditingCustomer ? (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    ...
                    <button onClick={saveCustomer} style={{
                      background: '#e8a33d', color: '#000', border: 'none', borderRadius: '8px',
                  ...
                  <>
                    {selectedCustomer.address && (
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                        ...
                        <a href={`https://maps.google.com/?q=${encodeURIComponent(selectedCustomer.address)}`} target="_blank" rel="noopener noreferrer" style={{ color: '#e8a33d', fontSize: '12px', textDecoration: 'none' }}>Navigate →</a>
                      </div>
                    )}
                    {selectedCustomer.phone && (
                      <a href={`tel:${selectedCustomer.phone}`} style={{ color: '#e8a33d', fontSize: '13px', textDecoration: 'none', display: 'block', marginBottom: '4px' }}>📞 {selectedCustomer.phone}</a>
                    )}
                  ...
                  {customerNotes.map(n => (
                    <div key={n.id} style={{ background: '#0f1729', borderRadius: '8px', padding: '10px 12px' }}>
                      ...
                      <div style={{ color: '#94a3b8', fontSize: '11px', marginTop: '4px', display: 'flex', gap: '8px' }}>
                        ...
                        {n.job_number && <span style={{ color: '#e8a33d' }}>· {n.job_number}</span>}
                      </div>
                    </div>
                  ))}
